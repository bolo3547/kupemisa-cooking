 ...
 * 
 * Flow:
 *  1. STATE_IDLE: Show "WAIT AUTH / Press A Login"
 ...
 * 10. Sale sent to dashboard (or queued), auto-logout, return to STATE_IDLE
 ********************************************************************/
...
// === PATCH A1: KEYPAD PIN FIX ===
// Avoid GPIO 32/33 (RTC/ADC pins unstable under WiFi + interrupts)
byte rowPins[ROWS] = {17, 5, 18, 19};
byte colPins[COLS] = {13, 16, 25, 27};
...
// ========================= STATE MACHINE =========================
enum State {
  STATE_IDLE,        // Wait for operator login (Press A)
...
void loadSettings() {
  ...
  // === PATCH B1: PRICE LOGGING ===
  Serial.printf("[PRICE] Loaded from NVS: %.2f %s/L\n", price.sell, price.currency);
...
bool httpPostJson(const char* path, const String& body, String& response) {
  ...
  if (!isOnline) {
    Serial.println("[HTTP] Not online - skipping request");
    return false;
  }
  if (!isCloudEnabled()) {
    Serial.println("[HTTP] Cloud not enabled - check API_KEY config");
    return false;
  }
  ...
  Serial.printf("[HTTP] Headers: device=%s, key=%s\n", DEVICE_ID, String(API_KEY).substring(0, 10).c_str());
...
bool fetchPriceFromDashboard() {
  ...
  // === PATCH B1: PRICE LOGGING ===
  Serial.printf("[PRICE] Updated from dashboard: %.2f %s/L\n", price.sell, price.currency);
  return true;
}
...
void sendTelemetry() {
  ...
  String safetyStatus = "STATE_IDLE";
  if (tamperActive) {
...
void connectWiFiNonBlocking() {
  if (String(WIFI_SSID).length() == 0) {
    ...
    lcdPrintCenter(1, "OFFLINE MODE");
    isOnline = false;
    Serial.println("[WIFI] No SSID configured - offline mode");
    return;
  }
...
void setup() {
  ...
  if (ENABLE_TAMPER_DETECTION) {
    ...
    Serial.println("[SECURITY] Tamper detection ENABLED - GPIO 34 requires external 10kO pull-up");
  } else {
...
void loop() {
  ...
  // Periodic WiFi status update and reconnection (non-blocking, when STATE_IDLE)
  if (state == STATE_IDLE && millis() - lastWifiCheckMs > WIFI_CHECK_MS) {
  ...
  // === PATCH B2: PERIODIC PRICE REFRESH (every 60s when idle + online) ===
  static uint32_t lastPriceCheck = 0;
  if (state == STATE_IDLE && isOnline && millis() - lastPriceCheck > 60000) {
  ...
  // === PATCH A4: TEMP TEST UNLOCK (REMOVE IN FINAL HARDENED BUILD) ===
  tamperLocked = false;
  tamperActive = false;
  
  // === PATCH A2: KEYPAD STABILITY ===
  // Requires >=70ms confirmation, prevents auto-repeat, uses millis()
  static char lastStableKey = 0;
  static uint32_t keyConfirmTime = 0;
  ...
  if (!tamperLocked) {
    ...
    if (!rawKey) {
      lastStableKey = 0;  // Key released
    } else if (rawKey != lastStableKey) {
      lastStableKey = rawKey;
      keyConfirmTime = now;  // Start confirmation timer
    } else if (now - keyConfirmTime >= 70) {
      k = rawKey;  // Confirmed after 70ms
      lastStableKey = 0;  // Consume until release (prevent repeat)
    }
  }
  ...
  if (k) {
    ...
    // Admin hold detection: pressing A in STATE_IDLE starts hold timer
    if (state == STATE_IDLE && k == 'A') {
      if (!holdingA) {
        holdingA = true;
        holdAStart = millis();
      }
    ...
    // ========== RECEIPT STATE ==========
      ...
      if (k == '#') {
        // Finish, auto-logout, return to STATE_IDLE
  ...
  if (!holdingA && holdAStart > 0 && state == STATE_IDLE) {
